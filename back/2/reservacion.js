...
async function habitacion(cantidadHuespedes, inicio, final, esCiclo = false) {
    ...
    try {
        ...
        const contenedorHabitaciones = document.getElementById('containerVisual');
        if (!esCiclo) {{
        ...
        for(let i = 0; i < data.length; i++) {
            ...
            contenedorHabitaciones.innerHTML += `<div
...
const contenedorServicios = document.getElementById('containerVisual');
async function servicio(inicio, final, cantidadHuespedes, categoria = null, esCiclo = false) {
    ...
    try {
        ...
        for(let i = 0; i < data.length; i++) {
            ...
            contenedorServicios.innerHTML += `<div class="bg-white rounded-xl overflow-hidden group border border-outline-variant/30 hover:shadow-lg transition-shadow duration-500">
...
document.getElementById('consultarServicios').addEventListener('click', (event) => {
    ...
    console.log(inicio)
    console.log(final)
    servicio(inicio, final, cantidadHuespedes[0])
})
...
document.getElementById('consultarHabitaciones').addEventListener('click', (event) => {
    event.preventDefault();
    const cantidadHuespedes = document.getElementById('selectorHuespedes').value;
    const inicio = new Date (document.getElementById('selectorInicio').value);
    const final = new Date (document.getElementById('selectorFinal').value);
